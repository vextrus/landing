...
export default function Procurement() {
  ...
  const features = [
    {
      ...
      description: 'ML-based vendor reliability prediction with 94% accuracy'
    },
    {
      icon: TrendingDown,
      title: 'Price Forecasting',
      description: 'Material cost prediction with monsoon impact modeling'
    },
    {
      icon: Link,
      title: 'Blockchain Tracking',
      description: 'Immutable supply chain verification from port to site'
    },
    {
      icon: FileText,
      title: 'Smart Contracts',
      description: 'Automated purchase orders with Bangladesh bank integration'
    }
  ]

  // Calculate aggregate metrics from suppliers data
  const savingsAmount = 234500000 // ৳23.45 Cr in savings
  const avgRating = suppliers.reduce((sum, s) => sum + (s.rating || 0), 0) / suppliers.length
  
  const metrics = [
    { 
      label: 'Cost Savings', 
      value: formatBDT(savingsAmount), 
      trend: '+23% YoY' 
    },
    { 
      label: 'Active Suppliers', 
      value: realtimeSuppliers.toString(), 
      trend: 'Verified' 
    },
    { 
      label: 'Avg Rating', 
  ...
  return (
    <ModuleWrapper
      moduleName="Smart Procurement Suite"
      moduleIcon={Package}
      moduleDescription="AI-driven supplier management with blockchain supply chain tracking"
      features={features}
      metrics={metrics}
      accentColor="#22C55E"
      directAccess={true}
